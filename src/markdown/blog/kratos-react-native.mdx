---
path: '/login-react-native-authentication-example-api/'

title: |
  Add Authentication to your React Native App

teaser: |
  Add login, registration, user, and profile management to React Native mobile
  and desktop apps using the Ory Kratos open source project. Includes code examples for TypeScript
  and Docker!
...
seo:
  ...
  description: |
    Add open source login to any React Native app using free open source.
...
Follow the step-by-step guide to add authentication to your
[React Native](https://reactnative.dev) application and screens for:
...
The examples use [Ory Kratos](https://github.com/ory/kratos), an open source
identity and authentication REST API server written in Golang.

import LazyLoad from 'react-lazyload'
import CodeFromRemote from '../../components/freestanding/utils/codefromremote'
import loginVideoMP4 from '../../images/articles/react-native/login-small.mp4'
import signupVideoMP4 from '../../images/articles/react-native/signup-profile-settings-small.mp4'
import loginVideoWebm from '../../images/articles/react-native/login.webm'
import signupVideoWebm from '../../images/articles/react-native/signup-profile-settings-small.webm'
...
<p>
  <LazyLoad height={500} once>
    <figure>
      ...
      <figcaption>
        Authentication for React Native Mobile App with Screens for Login,
        Registration and User Management API using Ory Kratos as a backend.
      </figcaption>
    </figure>
  </LazyLoad>
</p>
...
[Ory Kratos](https://www.ory.sh/kratos) project. The
[React Native](https://reactnative.dev) application shown in the screens below
is available on
...
You can download the React Native example app right now from the
...
This guide assumes that you have worked with [ReactJS](https://reactjs.org) and
React Native before as we will not cover React fundamentals and focus on
implementing login, registration, and so on.
...
Kratos at
[playground.projects.oryapis.com](https://playground.projects.oryapis.com/hosted/login).
...
your own managed environment reach out to [office@ory.sh](mailto:office@ory.sh)
or set up your own open source environment. Information on achieving that is
...
<p>
  <LazyLoad height={500} once>
    <figure>
      ...
      <figcaption>
        User Registration, Dashboard, Profile Management in React Native using
        Ory Kratos.
      </figcaption>
    </figure>
  </LazyLoad>
...
To manage identities, use the Ory Kratos CLI. An overview of all commands can be
found in the
[Ory Kratos documentation](https://www.ory.sh/kratos/docs/cli/kratos). Because
we are using the playground in this example, we will use the
[Ory CLI](https://github.com/ory/cli) instead as it allows to connect to Ory.
...
Ory Kratos does not issue JSON Web Tokens but instead so-called opaque Ory
Kratos Session Tokens. You can still convert those tokens to JSON Web Tokens if
you want, but Ory Kratos Session Tokens are more secure because:

1. **JSON Web Tokens can not hold secrets**: Unless encrypted, JSON Web Tokens
   can be read by everyone, including 3rd Parties. Therefore, they can not keep
   secrets.
2. **JSON Web Tokens can not be revoked / invalidated / logged out**: Well, you
   can revoke them, but they will be considered valid until the "expiry" of the
   token is reached. Unless, of course, you have a blacklist or check with Hydra
   if the token was revoked, which however defeats the purpose of using JSON Web
   Tokens in the first place.

## Run Ory Kratos Login, Registration, 2FA Server Locally in Docker

Instead of using the hosted demo environment, you can also deploy your Ory
...
The entry point for the app is
...
understanding how everything works even better. Let's have a look at the project
set up process!
...
Next we want to set up support for environment variables which we will use in
the next section. You can either
...
the [community forum](https://github.com/ory/kratos/discussions/) or
