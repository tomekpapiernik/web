...
- [Ory Hydra](https://github.com/ory/hydra) (started in 2015)
- [Ory Fosite](https://github.com/ory/fosite) (started in 2016)
...
Before reading on please note that the views stated in this article are **my
...
are two primary use cases where you are implementing OAuth2 / OpenID Connect and that this article only
covers one of them:
...
2. Being a provider of OAuth2 / OpenID Connect: You want to become the system
   that issues OAuth2 Access Tokens, OpenID Connect ID Tokens, etc. This
   article is for you!

Now that we've set the scene, let's look at the structure of this article. Similar
...
2. When are OAuth2 and OpenID Connect useful and what are their problems?
...
1. We have the utmost respect for the amount of innovation spawned by researchers
   and contributors in the area of OAuth2 and OpenID Connect. This article
   does not try to diminish their impressive work. Rather, we intend to shift the
...
2. Ory is developing an open-source identity infrastructure service called
   ...
   a brilliant community. We want something that works for 80% of use cases.
...
A generic decision tree to help you decide whether you need OAuth2 or not can be
...
<p>
  <iframe
    width="768"
    height="432"
    src="https://miro.com/app/live-embed/uXjVOYIT_dA=/?moveToViewport=-449,-1108,4244,2070"
    frameBorder="0"
    scrolling="no"
    allowFullScreen
  ></iframe>
...
Since you're reading this on the Ory website and we believe in the products we
build, you will find those prominently in the above graph. For comparable products we
have collected the following **open source** ones:

1. If you need both an identity platform, implying that you most likely need to
   ...
   [RedHat's Keycloak](https://www.keycloak.org). KeyCloak is as if you were to
   use Ory's Identity Platform and Ory's OAuth2 / OpenID Connect platform
   together, while only using OAuth2 & OpenID Connect to "authenticate".
2. If you only need an identity platform without OAuth2 there are a few
   alternatives to Ory's Identity Platform /
   [Ory Kratos](https://github.com/ory/kratos):
   1. Language-specific libraries such as
      [authboss](https://github.com/volatiletech/authboss) with the up- and
      downsides language-specific libraries have.
   2. ~~[Supertokens](https://supertokens.io)~~ - we no longer endorse them due to
      a questionable choice of mixing OAuth2 terminology with self-built
      ...
      [Scott Brady calls MyOwnOAuth™](https://www.scottbrady91.com/blockchain-identity/technical-review-of-civics-secure-identity-platform).
...
Connect are the best-in-class protocols to address your use case!
...
<p>
  <small>
    Google does not use OAuth2 or OpenID Connect in their login process
  </small>
</p>

A counterexample to this is Amazon which uses OpenID Connect to sign you into
their Amazon store.

![Amazon uses OpenID Connect](../../images/articles/oauth2-use-cases-examples/amazon-oidc.png)

<p>
  <small>Amazon uses OAuth2 / OpenID Connect in their login process</small>
</p>

#### Niche Clients

Another exception is if you plan interaction with a lot of different client
types. This in particular applies to clients that do not have a traditional user interface
...
- SmartTVs because they do not have a keyboard, mouse, or USB port (for e.g.
  YubiKeys), making log in and sign up frustrating or impossible
- Command Line Interfaces when they should use your beautiful website and login
  forms (with multiple authentication factors) to sign users in

#### Integration with Many Other Services

Adding OAuth2 and OpenID Connect to your system, where you're not using it as the primary
authentication system for your own services, is beneficial if you use other
tools and services which should use the central identity platform to
authenticate users.

Examples of these tools could be:

- [Rocket Chat](https://docs.rocket.chat/guides/administration/misc.-admin-guides/authentication/open-id-connect)
  where users sign up at a service and are able to also enter a chat room by
  logging into Rocket Chat using your OpenID Connect server.
- SaaS tools that offer Enterprise SSO plans, like
  [Typeform](https://www.typeform.com/help/a/set-up-sso-authentication-for-your-account-for-enterprise-customers-360060078751/).
- Other software products offering SSO via OpenID Connect.

In these cases you have to weigh whether you want to use one authentication
...
The decision here is not clear-cut and depends a lot on context. It can be as
difficult to implement two authentication mechanisms for first and third-party
use as implementing only OAuth2 for first and third-party.
...
This list is long, but there is an even longer list of draft and active development RFCs and specifications. Alongside these more generic specs, there are
also specs for financial institutions (FAPI), governments (iGov), EAP, MODRNA,
and many more.

So the question remains: do you really need all of this? Or should we simply solve
login and move on until we actually need to address a critical need?

## OAuth2 is Difficult in Some Use Cases
...
>    identity provider such as Facebook or Google, and they already have a
...
access to your GitHub or GitLab repositories which means it uses GitHub's / GitLab's
OAuth2 capabilities.

The catch is if you signed in to CircleCI via GitHub, and you log out
of GitHub. You will still be signed in to CircleCI! The same applies if you use
an app using "Sign in with {anything really}". With OAuth2 it does not matter to the third
party what your session status is at the first party!

At Ory, we get (and answer) this question on a regular basis: _"I have implemented
...
1. Cookies which only can be set by the server (`httpOnly`).
...
There are several opinions and considerations to be made. We've collected a few of
the most interesting discussions for further reading:
...
iteration to make the right decisions.
...
company of almost 30 people (a comparatively small number).
Throughout this, I've witnessed the community grow and struggle with OAuth2 and OpenID
Connect terminology, complexity, and tooling. In hindsight, it's clear that
scaling development (the human side) with complex protocols is challenging since
...
has to be taught and understood by any new team member touching these systems. Sure, there are tons
...
OpenID Connect is whether you want users to see a screen similar to the following
one:

![GitHub OAuth2 Prompt Screen](../../images/articles/oauth2-use-cases-examples/github-oauth2-prompt.png)
...
Specifications are subject to change and deprecation as any other system. RFCs
...
"API Keys" in the wild. These types of tokens are not the result of OAuth2 flows!
...
You can use JSON Web Tokens without OAuth2. These are two different standards
which can be used independently! See the section above.
...
access to your system. Here too it might make sense to start using OAuth2 and OpenID
Connect immediately to maintain one standard method of authentication.

If you are a small team, a start-up, or a company with a single product you will
...
amount of time and development resources to get it right.
...
Do you have a different opinion on this topic or did you just enjoy the read? Write me on
[Twitter](https://twitter.com/_aeneasr) or follow me on
[GitHub](http://github.com/aeneasr).

Thank you for reading and see you next time! Maybe in the
