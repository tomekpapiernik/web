...
are two primary use cases when implementing OAuth2 / OpenID Connect and this article only
covers the latter:
...
Example for niche clients can be:
...
Adding OAuth2 and OpenID Connect to your system, so not using it as the primary
authentication system for your own services, is beneficial if you use other
tools and services and they should use the central identity platform to
authenticate users.

Examples for can be tools like:
...
- A lot of SaaS tools that offer Enterprise SSO plans, like
...
parties, or whether you want to use OpenID Connect for both. The decision here
...
You have a different opinion on this topic or enjoyed the read? Write me on
