...
that this article only covers one of them:
...
Now that we've set the scene, let's look at the structure of this article.
...
regularly spawn new, innovative ones.
...
1. We have the utmost respect for the amount of innovation spawned by
...
2. Ory is developing an open-source identity infrastructure service called
   ...
   brilliant community. We want something that works for 80% of use cases.
...
products we have collected the following **open source** ones:
...
2. If you only need an identity platform without OAuth2 there are a few
   ...
   2. ~~[Supertokens](https://supertokens.io)~~ - we no longer endorse them due
...
Adding OAuth2 and OpenID Connect to your system, where you're not using it as
...
This list is long, but there is an even longer list of draft and active
...
MODRNA, and many more.

So the question remains: do you really need all of this? Or should we simply
solve login and move on until we actually need to address a critical need?
...
>    information with Auth0 and, in turn, your application.
...
GitLab's OAuth2 capabilities.

The catch is if you signed in to CircleCI via GitHub, and you log out of GitHub.
...
third party what your session status is at the first party!

At Ory, we get (and answer) this question on a regular basis: _"I have
implemented OAuth2 but how do I globally sign out my users?"_.
...
There are several opinions and considerations to be made. We've collected a few
of the most interesting discussions for further reading:
...
there are tons of resources out there. But which one **applies to your system**?
...
OpenID Connect immediately to maintain one standard method of authentication.
...
Do you have a different opinion on this topic or did you just enjoy the read?
Write me on [Twitter](https://twitter.com/_aeneasr) or follow me on
